...
void TVout::set_invert(uint8_t i) {
	invert = i;
}

void TVout::set_mirror(uint8_t i) {
	mirror = i;
}

void TVout::enable_cursor(uint8_t i) {
	show_cursor = i;
}
...
void TVout::print_char(uint8_t x, uint8_t y, unsigned char c) {
	...
	bitmap(x,y,font,(c*pgm_read_byte(font+1))+3,pgm_read_byte(font),pgm_read_byte(font+1), invert);
}
...
void TVout::write(uint8_t c) {
	...
	if (show_cursor && cursor_x <= (display.hres*8 - pgm_read_byte(font))) {
		print_char(cursor_x, cursor_y, '_');
	}
}
...
void TVout::set_char_cursor(uint8_t x, uint8_t y) {
	cursor_x = pgm_read_byte(font)*x;
	cursor_y = pgm_read_byte(font+1)*y;
}
